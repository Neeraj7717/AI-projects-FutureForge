...
# Ensure that the landmarks contain the 'visibility' attribute
def validate_landmarks(landmarks):
    for landmark in landmarks:
        if 'visibility' not in landmark:
            raise ValueError("Landmark dictionary must contain 'visibility' key.")

# Validate landmarks before processing
def validate_and_extract_landmarks(landmarks):
...
pose_analytics = logging.getLogger('pose_analytics')
pose_analytics.setLevel(logging.INFO)
console_handler = logging.StreamHandler()
log_format = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
console_handler.setFormatter(log_format)
pose_analytics.addHandler(console_handler)

ENABLE_FILE_LOGGING = False
if ENABLE_FILE_LOGGING:
    file_handler = logging.FileHandler('pose_analytics.log')
    file_handler.setFormatter(log_format)
    pose_analytics.addHandler(file_handler)
pose_analytics.propagate = False
...
if not os.path.exists('/tmp'):
    try:
        os.makedirs('/tmp')
        pose_analytics.info("Created /tmp directory")
    except OSError as e:
        pose_analytics.error(f"Failed to create /tmp directory: {e}")
...
async def upload_bad_squat_frame(session_id, manual_id, frame):
    try:
        ...
        if not success:
            pose_analytics.error(f"cv2.imwrite failed to write image to {local_path}")
            return None
...
def update_value(redis_client, sessionId, manualId, key, new_value):
    ...
    if redis_client.exists(redis_key):
        ...
        pose_analytics.info(f"Key '{redis_key}' does not exist. Cannot update.")

def analyze_live_squat(session_id, manual_id, frame, landmarks, target_reps=1):
    pose_analytics.info("----------------------((((((((((((((((((((((((( In squat )))))))))))))))))))))))))--------------------------------")
    ...
    # print("Landmarks:", landmarks, "\n", "Results", type(landmarks))

    if landmarks:
        landmarks = validate_and_extract_landmarks(landmarks)
        ...
        all_visible = all(landmarks[lm.value]['visibility'] > visibility_threshold for lm in required_landmarks)
        
        if not all_visible:
            pose_analytics.info("Skipping frame - not all required landmarks are visible")
        ...
        print("Min kenn angel", current_knee_angle)
    ...
    pose_analytics.info("redis feedback------------------------------------------- %s", redis_feedback)
    pose_analytics.info("redis feedbackUrl------------------------------------------- %s", redis_feedback_url)

    pose_analytics.info("----------------------((((((((((((((((((((((((( Out squat )))))))))))))))))))))))))--------------------------------")
...
def close_redis_connection_pose_utils():
    ...
    """
    redis_client.close()
    pose_analytics.info("Redis connection closed.")