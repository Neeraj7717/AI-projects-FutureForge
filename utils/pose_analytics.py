...
def create_3d_pose_frame(pose_3d, elev=20, azim=70):
    """
    Create a single 3D pose visualization frame
    """
    ...
    # Plot the joints
    ax.scatter(pose_3d[:, 0], pose_3d[:, 2], pose_3d[:, 1], c='blue', s=10)
    
    # Plot the connections
    for connection in POSE_CONNECTIONS:
        start_idx, end_idx = connection
        if start_idx < len(pose_3d) and end_idx < len(pose_3d):
            ax.plot([pose_3d[start_idx, 0], pose_3d[end_idx, 0]],
                    [pose_3d[start_idx, 2], pose_3d[end_idx, 2]],
                    [pose_3d[start_idx, 1], pose_3d[end_idx, 1]], 'r-', linewidth=1)
    ...
    # Reshape and convert from RGBA to RGB
    img = buffer.reshape(height, width, 4)[:, :, :3]
...
def analyze_live_squat(session_id, frame, target_reps=1):
    ...
    with mp_pose.Pose(
        ...
        # Initialize 3D visualization frame
        pose_3d_img = np.zeros((frame_height, frame_width, 3), dtype=np.uint8)
        ...
        if results.pose_landmarks:
            ...
            if current_knee_angle > 130 and is_squatting:
                is_squatting = False
                total_squat_count += 1
