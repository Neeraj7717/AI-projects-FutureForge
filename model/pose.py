...
class Pose:
    ...
    def send_instruction_pose(self,xyxy,new_width,new_height,sourceId,sessionId,manualId,things_present,keypoints=[]):
        ...
        try:
            ...
            traceback.print_exc()
            pass
        pose_logger.info({"sessionId":sessionId,"manualId":manualId,"sourceId":sourceId,"thingsPresent":things_present})
    ...
    def pose_detector(self, file, sourceId, sessionId, manualId):
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1000) as executor:
                executor.submit(self.draw_annotations(frame, results.pose_landmarks, sourceId, sessionId, manualId))
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1000) as executor:
                executor.submit(self.instruction_graph(sourceId, sessionId, manualId, frame, things_present))
            
            total_time = time.time() - start_time
            pose_logger.info(f"Total pose detection pipeline completed in {total_time:.3f}s")
    ...
    def draw_annotations(self, image, landmarks, sourceId, sessionId, manualId):
        ...
        for connection in skeleton_connections:
            ...
            # pose_logger.info(f"Drawing line from {connection[0].name} to {connection[1].name}")
            # pose_logger.info(f"Coordinates: ({start_x}, {start_y}) to ({end_x}, {end_y})")
