      ...
      try {
        ...
        if (patchRequest != null) {
          ...
          if (patchRequest.getProperties() != null) {
            ...
            List<PropertyDAO> existingProperties =
                PropertyRepository.findProperties(session, tableInfoDAO.getId(), Constants.TABLE);
            Map<String, PropertyDAO> existingPropMap =
                existingProperties.stream().collect(Collectors.toMap(PropertyDAO::getKey, p -> p));
            ...
            for (Map.Entry<String, String> entry : requestProperties.entrySet()) {
              ...
              if (existingProp != null) {
                ...
                if (!Objects.equals(value, existingProp.getValue())) {
                  ...
                  session.merge(existingProp); // Merge updates to existing entity
                  updated = true;
                }
              } else {
                // Create a new property and use merge to handle it
                LOGGER.debug("Adding new property: {} = {}", key, value);
                PropertyDAO newProp =
                    PropertyDAO.builder()
                        .id(UUID.randomUUID()) // Assign a new UUID for new properties
                        .entityId(tableInfoDAO.getId())
                        .entityType(Constants.TABLE)
                        .key(key)
                        .value(value)
                        .build();
                session.merge(newProp); // Use merge instead of persist
                updated = true;
              }
            }
          ...
          if (patchRequest.getStorageLocation() != null) {
            if (TableType.MANAGED.equals(tableInfoDAO.getType())) {
              throw new BaseException(
                  ErrorCode.INVALID_ARGUMENT, "Cannot update storage location for MANAGED tables");
            }
            tableInfoDAO.setUrl(
                FileOperations.convertRelativePathToURI(patchRequest.getStorageLocation()));
            updated = true;
          }
          ...
          if (updated) {
            ...
            session.merge(tableInfoDAO); // Merge table updates
        ...
        // Fetch the updated entity to ensure we return the latest state
        ...
        RepositoryUtils.attachProperties(
            updatedTableInfo, tableInfoDAO.getId().toString(), Constants.TABLE, session);
        ...
        throw e instanceof BaseException
            ...
            : new BaseException(
                ErrorCode.INTERNAL,
                "Error updating table: " + fullName + ". Cause: " + e.getMessage(),
                e);
      }
    }
  }
