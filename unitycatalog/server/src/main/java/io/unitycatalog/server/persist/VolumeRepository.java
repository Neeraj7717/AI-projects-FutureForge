...
public class VolumeRepository {
    ...
    private static final PagedListingHelper<VolumeInfoDAO> LISTING_HELPER = new PagedListingHelper<>(VolumeInfoDAO.class);
    ...
    public VolumeInfo createVolume(CreateVolumeRequestContent createVolumeRequest) {
        ...
        String volumeFullName =
            createVolumeRequest.getCatalogName() + "." + createVolumeRequest.getSchemaName() + "." + createVolumeRequest.getName();
        ...
        // Handle properties
        if (createVolumeRequest.getProperties() != null) {
            volumeInfo.setProperties(new HashMap<>(createVolumeRequest.getProperties()));
        }
        ...
        try (Session session = sessionFactory.openSession()) {
            ...
            try {
                SchemaInfoDAO schemaInfoDAO = repositories
                    .getSchemaRepository()
                    .getSchemaDAO(session, createVolumeRequest.getCatalogName(), createVolumeRequest.getSchemaName());
    ...
    public VolumeInfo getVolume(String fullName) {
        try (Session session = sessionFactory.openSession()) {
            ...
            return convertFromDAO(getVolumeDAO(session, catalogName, schemaName, volumeName), catalogName, schemaName);
        } catch (Exception e) {
    ...
    public VolumeInfoDAO getVolumeDAO(Session session, UUID schemaId, String volumeName) {
        Query<VolumeInfoDAO> query = session.createQuery(
            "FROM VolumeInfoDAO WHERE name = :name and schemaId = :schemaId",
            VolumeInfoDAO.class
        );
    ...
    public ListVolumesResponseContent listVolumes(
        ...
        try (Session session = sessionFactory.openSession()) {
            ...
            try {
                ...
                ListVolumesResponseContent responseContent = listVolumes(session, schemaId, catalogName, schemaName, maxResults, pageToken);
                tx.commit();
    ...
    private VolumeInfo convertFromDAO(VolumeInfoDAO volumeInfoDAO, String catalogName, String schemaName) {
        ...
        if (volumeInfoDAO.getProperties() != null) {
            volumeInfo.setProperties(new HashMap<>(volumeInfoDAO.getProperties()));
        } else {
            volumeInfo.setProperties(new HashMap<>());
        }
    ...
    public VolumeInfo updateVolume(String fullName, UpdateVolumeRequestContent updateVolumeRequest) {
        LOGGER.debug("Updating volume: {}", fullName);
        try (Session session = sessionFactory.openSession()) {
            Transaction tx = session.beginTransaction();
            try {
                String[] namespace = fullName.split("\\.");
                ...
                if (updateVolumeRequest != null) {
                    if (updateVolumeRequest.getNewName() != null) {
                        ...
                        VolumeInfoDAO existingVolume = getVolumeDAO(session, catalog, schema, updateVolumeRequest.getNewName());
                    ...
                    if (updateVolumeRequest.getProperties() != null) {
                        volumeInfo.setProperties(new HashMap<>(updateVolumeRequest.getProperties()));
                        updated = true;
                    }
                ...
                VolumeInfo updatedVolumeInfo = convertFromDAO(volumeInfo, catalog, schema);
                tx.commit();
                ...
                throw new BaseException(ErrorCode.INTERNAL, "Failed to update volume: " + fullName + ". Cause: " + e.getMessage(), e);
            }
        }
    }
    ...
    public void deleteVolume(String name) {
        try (Session session = sessionFactory.openSession()) {
            ...
            Transaction tx = session.beginTransaction();
            try {
                SchemaInfoDAO schemaInfo = repositories.getSchemaRepository().getSchemaDAO(session, catalog, schema);
                if (schemaInfo == null) {
